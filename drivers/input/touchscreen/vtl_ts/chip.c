...
static int chip_i2c_read(struct i2c_client *client, __u16 addr, __u8 *buf,
			 __u16 len)
{
	struct i2c_msg msgs;
	int ret;
	...
	if (ret != 1) {
		printk("___%s:i2c read error: %d\n", __func__, ret);
		return -1;
	}
	return 0;
}

static int chip_i2c_write(struct i2c_client *client, __u16 addr, __u8 *buf,
			  __u16 len)
{
	struct i2c_msg msgs;
	int ret;
	...
	ret = i2c_transfer(client->adapter, &msgs, 1);
	if (ret != 1) {
		printk("___%s:i2c write error: %d\n", __func__, ret);
		return -1;
	}
	return 0;
}
...
int chip_function(enum cmd_index cmd_index, unsigned char *rx_buf,
		  unsigned char len)
{
	int ret = 0;
...
int chip_get_checksum(struct i2c_client *client, int *bin_checksum,
		      int *fw_checksum)
{
	DEBUG();
...
static int chip_write_infoblk(struct i2c_client *client)
{
	...
	for (cod = 0; cod < 16; cod++) {
		...
		buf[5] =
			(~(buf[2] + buf[3] + buf[4] + buf[6] + buf[7] + buf[8] +
			   buf[9] + buf[10] + buf[11] + buf[12] + buf[13])) +
			1;
